...
(async () => {
  ...
  try {
    ...
    const pTok = (await req('POST','/api/auth/login', null, { username:'planner', password:'test' })).body.token;
    const nTok = (await req('POST','/api/auth/login', null, { username:'normalo', password:'test' })).body.token;
