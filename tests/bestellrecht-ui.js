...
//
// Seit 08.09.2026 gilt dasselbe für „Lagerdaten pflegen" (can_products_edit) — mit dem UMGEKEHRTEN
...
(async () => {
  ...
  try {
    ...
    ok('Buchhalter: Lagerdaten-Kästchen bleibt sichtbar', await sichtbar(a.seite, '#um-can-products-edit'));
    ...
    ok('Chef: auch Lagerdaten verborgen (er darf per Rolle)', !(await sichtbar(a.seite, '#um-can-products-edit')));
    await a.seite.select('#um-role', 'mitarbeiter'); await sleep(400);
    ok('Mitarbeiter: beides wieder da',
      (await sichtbar(a.seite, '#um-can-order')) && (await sichtbar(a.seite, '#um-can-plan')));
    ok('Mitarbeiter: Lagerdaten-Kästchen ist da', await sichtbar(a.seite, '#um-can-products-edit'));
    ...
    await a.seite.click('#um-can-products-edit');
    ...
    ok('… und das Lagerdaten-Recht ebenso', gespeichert && gespeichert.can_products_edit === 1,
      JSON.stringify(gespeichert && gespeichert.can_products_edit));
