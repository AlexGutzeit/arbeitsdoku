...
function authenticate(req, res, next) {
  ...
  try {
    ...
    const db = getDb();
    const user = db.prepare('SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload FROM users WHERE id = ?').get(decoded.userId);
