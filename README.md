...
**Drei Ansichten** (Knöpfe über dem Board): **Mitarbeiter** (die gewohnte Sicht – Spalte je Mitarbeiter plus „Nicht zugewiesen"), **Kategorien** (Spalte je Auftrags-Kategorie plus „Ohne Kategorie") und **Alle** (Mitarbeiter *und* Kategorien nebeneinander). Die zuletzt gewählte Ansicht bleibt beim Blättern erhalten – und wird **auf dem Gerät gemerkt**: nach dem Schließen und Wiederöffnen der App steht wieder derselbe Reiter an (bewusst geräte- und nicht kontobezogen, weil man am Rechner anders arbeitet als am Handy). Auch die **waagerechte Scroll-Stelle** bleibt: Wer weit rechts an einer Kategorie arbeitet, einen Auftrag bearbeitet und speichert oder abbricht, steht danach wieder genau dort. In **Alle** steht in „Nicht zugewiesen" nur, was **weder** Mitarbeiter **noch** Kategorie hat – ein Auftrag mit Kategorie taucht unter dieser auf. Dieselbe Kachel kann dadurch mehrfach erscheinen (einmal beim Mitarbeiter, einmal bei jeder Kategorie); jede Spalte ist so für sich vollständig. Die **Plaketten** unter dem Kundennamen zeigen dabei **alles, woran der Auftrag hängt, außer der Spalte, in der man gerade steht** – Mitarbeiter mit ihrem Farbpunkt, Kategorien grau. Ein Auftrag an Max und Anna in „PV" und „Zählerschrank" zeigt also unter *Max* die Plaketten *Anna · PV · Zählerschrank*, unter *PV* die Plaketten *Max · Anna · Zählerschrank* und so fort. So sieht man auf jeder Kachel auch, **mit wem** man sie teilt.

**Auftrags-Kategorien** (z. B. „Kleinarbeiten", „PV", „Zählerschrank"): **Chef/Admin** legen sie direkt im Board an (Knopf **„+ Kategorie"** in der Kategorie-Ansicht), benennen sie über **✎** im Spaltenkopf um oder löschen sie über **×**. **Löschen fragt einmal nach** und nennt dabei, wie viele Aufträge betroffen sind („An dieser Kategorie hängen noch 3 Aufträge. Die Aufträge selbst bleiben erhalten …"); die Aufträge bleiben unangetastet, nur die Zuordnung fällt weg. Nur wenn die Zahl auf dem Bildschirm veraltet war (jemand hat inzwischen zugeordnet), kommt eine zweite Frage mit der richtigen Zahl. Namen sind eindeutig: „PV" und „p.v." gelten als dasselbe und werden abgewiesen. Ein Auftrag kann **mehreren Kategorien** zugeordnet sein (Häkchen im Auftrags-Formular) und gleichzeitig mehreren Mitarbeitern – beides ist voneinander unabhängig. Kategorien ohne Auftrag bleiben sichtbar, damit niemand sie versehentlich ein zweites Mal anlegt. *(Auftrags-Kategorien haben nichts mit den **Produkt-Kategorien** im Lager zu tun – zwei getrennte Listen für zwei verschiedene Dinge.)*

**Wenn jemand die Firma verlässt:** Ausgestellte Mitarbeiter behalten ihre Aufträge – nichts fällt still auf „Nicht zugewiesen". Ihre Spalte bleibt stehen, solange ihr noch etwas zugewiesen ist, wird mit **„ausgeschieden"** gekennzeichnet und ans **Ende** sortiert; ist der letzte Auftrag umverteilt oder erledigt, verschwindet sie von allein. In der **Zuteilung** (neuer wie bestehender Auftrag) stehen nur **aktive** Mitarbeiter zur Auswahl – eine bestehende Zuweisung an einen Ausgeschiedenen bleibt sichtbar und erhalten, bis sie jemand bewusst entfernt.
...
immer steht, was tatsächlich bestellt wurde. Wer sich vertippt hat, nimmt erst zurück und ändert dann.
...
**„Artikel einlernen"**. Wer **„Lagerdaten pflegen"** darf, legt ein Produkt aber
...
Benutzerverwaltung (anlegen/bearbeiten/Rolle setzen), **Soll-Stunden pro Woche** (mit Historie), Start-Überstunden, Passwort zurücksetzen, Einzelrechte vergeben – darunter **Bestellungen abschließen**: Damit darf auch ein Mitarbeiter (z. B. Vorarbeiter oder Urlaubsvertretung) offene Bestellungen auf *Bestellt* setzen (und das mit *Doch nicht bestellt* zurücknehmen) und fremde Einträge korrigieren, was sonst Chef, Admin und Buchhalter vorbehalten ist. Bestellte Einträge löschen bleibt beim Admin. **Zähler und Push-Meldung folgen dem Recht** – sonst hätte man den Knopf, erführe aber nie, dass etwas zu bestellen ist. Das gilt **ausnahmslos für jeden, der bestellen darf**, den Buchhalter eingeschlossen: Wer den Knopf hat, bekommt auch den Zähler im Menü und die Push-Meldung. Wird das Recht entzogen (auch durch einen Rollenwechsel), schaltet der Server den Push-Schalter für Bestellungen ab und streicht die Kategorie aus geplanten Zusammenfassungen; bleibt dort keine Kategorie übrig, wird die Zusammenfassung gelöscht und der Vorgang protokolliert. **Lagerdaten pflegen**: Damit darf auch ein Mitarbeiter das Produktverzeichnis aufräumen – umbenennen, Kategorie wechseln, Barcodes umhängen oder entfernen, doppelte Einträge zusammenführen und löschen. Sonst können das nur Chef und Admin; der **Buchhalter hat es anders als beim Bestellen *nicht* per Rolle** – mit dem Lager hat er nichts zu tun, wer ihn trotzdem braucht, bekommt das Häkchen wie jeder andere. **Artikel einlernen**: eigenes, kleineres Recht für den Moment am Regal – einen unbekannten Barcode ins Verzeichnis aufnehmen. Auch hier hat der Buchhalter es *nicht* per Rolle. Wer „Lagerdaten pflegen" hat, hat es automatisch mit; das Häkchen bleibt dann leer. Ohne das Recht endet ein unbekannter Scan mit einer Auskunft statt einer Eingabemaske – **bestellen** geht unverändert als freier Text weiter. (Bis 13.09.2026 durfte *jeder* anlegen, mit der Begründung, wer im Lager nichts eintragen kann, umgehe die App. Die Abwägung ist zugunsten der Datenhygiene umgedreht worden.) **Arbeitsbeginn** je Mitarbeiter (optional, leer = Firmenwert aus den Einstellungen) — nur für Ausnahmen, die früher oder später anfangen. **Geburtsdatum** (optional): daraus entscheidet die App, ob das Arbeitszeitgesetz oder das Jugendarbeitsschutzgesetz gilt — für die Pausen-Vorbelegung **und** für die gesetzlichen Warnungen (Tages- und Wochengrenze, Ruhezeit). Gerechnet wird das Alter **am Tag des Eintrags**, nicht am heutigen. **Leer heißt „unter 18"**, nicht „unbekannt, also Erwachsener"; ein Hinweis am Feld sagt das. Zukünftige und unmögliche Daten werden abgewiesen, jede Änderung steht im Protokoll. Bei allen Stunden-/Tage-Feldern sind „**,**" und „**.**" als Dezimaltrenner erlaubt (z. B. `7,5`); eine unlesbare Eingabe wird **gemeldet** statt still als 0 gespeichert. **Passwort-Anforderungen** (beim Anlegen & Zurücksetzen, serverseitig erzwungen): mind. 8 Zeichen, je 1× Groß-/Kleinbuchstabe, Ziffer und Sonderzeichen, und ≠ Benutzername – die Bedingungen werden beim Tippen **live** angezeigt (grüner Haken/rotes ✗, Feld färbt sich rot/grün). Bestehende Anmeldungen bleiben davon unberührt. **Urlaubsanspruch (versioniert):** je Mitarbeiter lassen sich – analog zu den Soll-Stunden – **mehrere Anspruchszeilen** „X Tage, **gültig ab** Datum" per „+" anlegen und rückwirkend ändern/löschen (z. B. Start-Anspruch bei Eintritt, Erhöhung im 2. Jahr). **Ohne Eintrag zählt 0** (kein Default), und solange keine Zeile existiert, bleibt überall die **alte Ansicht** (nur genommene Tage) – die Resturlaub-Anzeigen erscheinen erst mit hinterlegtem Anspruch. Jede Zeile trägt ihre **eigene Verfall-Regel** – Resturlaub verfällt **nie / zum Jahreswechsel / an einem Datum im Folgejahr** –, sodass sich der Modus über die Zeit **umstellen lässt, ohne die Vergangenheit zu verändern** (ein Moduswechsel wirkt nur ab dem jeweiligen „gültig ab" vorwärts). Der Anspruch ist **jahresbezogen**: pro Jahr gilt die Zeile mit dem **jüngsten „gültig ab" bis Jahresende** (mehrere Zeilen im selben Jahr → die späteste gewinnt); die Rechnung beginnt am **frühesten** „gültig ab"-Jahr, nicht bei der Mitarbeiter-Anlage. Ein Feld **„Start-Resturlaub (Übertrag)"** trägt den Stand **vor** Einführung der App als einmaligen Übertrag ins erste erfasste Jahr (analog Start-Überstunden) – so lassen sich Bestands-Mitarbeiter ohne Nachtragen aller Vorjahre übernehmen. Eine Stand-Anzeige mit Jahr-Auswahl zeigt sofort genommen/geplant/verbleibend. **Ausstellen statt Löschen:** ausgeschiedene Mitarbeiter werden mit Austrittsdatum ausgestellt, ihre Zeiten/Abwesenheiten/Planungen bleiben aber vollständig erhalten und für ihren Anstellungszeitraum in Statistik und PDF berücksichtigt. Wiedereinstellen ist jederzeit möglich (auch mehrfach) – die Lücke zählt 0 Soll-Stunden.

**Überstunden auszahlen:** Beim Ausscheiden – oder wenn am Jahresende schlicht zu viel aufgelaufen ist – gibt es drei Wege: **abfeiern, stehen lassen, auszahlen**. Für den dritten steht bei jedem Mitarbeiter ein Knopf **„Überstunden auszahlen"**; der Ausstellen-Dialog nennt den Stand und fragt ausdrücklich danach. Der Chef trägt Stunden und Stichtag ein, **entscheiden tut der Mitarbeiter selbst** auf *Mein Konto* (Zähler am Menüpunkt, live). Bis er zustimmt, bewegt sich **keine Zahl** – die Anfrage wird nur als offen ausgewiesen, damit niemand dieselben Stunden zweimal verplant. Ablehnen geht mit Begründung, zurückziehen kann der Chef, solange nichts entschieden ist. **Auch Chef und Admin können nicht für ihn zustimmen:** Der Verzicht auf Stunden ist seine Entscheidung, keine Verwaltungshandlung. Liegt die Zustimmung unterschrieben vor, trägt der Chef sie als solche ein – dieser Weg ist dann **überall gekennzeichnet** („per Unterschrift"), im Protokoll wie in der Anzeige.

Ein Stichtag in einem **bereits abgeschlossenen Monat** wird auf den ersten noch offenen Tag **gezogen**, nicht rückwirkend verbucht (der gewünschte Tag steht im Protokoll) – ein Abschluss, der sich nachträglich bewegt, wäre das Gegenteil von revisionssicher. Mehr Stunden als vorhanden werden **gewarnt, aber zugelassen**: Ein Minus kann gewollt sein. Das **Ausstellen wird nie blockiert** – Überstunden mitnehmen ist ein normaler Ausgang; eine beim Vollzug noch offene Anfrage wird automatisch zurückgezogen (protokolliert), damit sie nicht ewig hängt, denn zustimmen kann nur der Betroffene selbst.

**Der Chef erfährt, was daraus wurde:** Am Menüpunkt *Mitarbeiter* steht ein **Zähler**, sobald ein Mitarbeiter entschieden hat — ohne ihn hätte niemand einen Anlass, dort überhaupt nachzusehen. In der Liste erscheint die Meldung „Auszahlung abgelehnt" bzw. „bestätigt" (mit der Begründung als Kurzinfo); sie verschwindet, **sobald der Chef die Liste einmal geöffnet hat**. Ein noch **offener** Hinweis bleibt dagegen stehen, solange er gilt — das ist ein Zustand, keine Meldung. Nachlesbar bleibt alles im aufklappbaren **Verlauf** unter der Liste: Stunden, Stichtag, Stand, wer angelegt und wer entschieden hat, samt Begründung. Jede Aktion steht zusätzlich im **Protokoll** (angelegt, bestätigt, abgelehnt, zurückgezogen, per Unterschrift, beim Ausstellen zurückgezogen).

Ausgewiesen wird die Auszahlung im **Lohn-Export in eigenen Spalten** („Auszahlung Stunden", „Auszahlung Beleg") – bewusst **nicht** in den Nachtrags-Spalten, die etwas anderes bedeuten: Ein Nachtrag heißt „hier fehlten Stunden", eine Auszahlung heißt „diese Stunden sind mit Geld abgegolten". In der Statistik erscheint „davon ausgezahlt", damit ein gesunkener Stand erklärbar bleibt, und auf *Mein Konto* steht der Verlauf. **Kein Geldbetrag und kein Stundensatz:** Die App liefert Stunden, das Lohnbüro rechnet.

**Liegt der letzte Arbeitstag noch in der Zukunft, wird der Austritt *vorgemerkt*:** Das Datum steht sofort fest und begrenzt die Soll-Stunden, der **Zugang bleibt aber bis einschließlich diesem Tag bestehen** — der Mitarbeiter kann bis zuletzt seine Stunden buchen. In der Nacht danach schließt die App das Konto von selbst und löscht erst dann Zwei-Faktor und Push-Abos. In der Mitarbeiterliste steht bis dahin *„scheidet aus zum …“*, und die Vormerkung lässt sich mit einem Klick wieder aufheben. Ein Austrittsdatum von **heute oder früher** wirkt wie bisher **sofort** — auch das Ausstellen ohne Datumsangabe, und eine laufende Sitzung fliegt dann sofort heraus. *Der Mitarbeiter sieht eine Vormerkung nicht:* Weder in der App noch in „Meine Daten“ erscheint ein noch nicht erreichtes Austrittsdatum — er soll nicht aus der Software von seiner Kündigung erfahren. Die formale **Datenauskunft** enthält den Zeitraum dagegen vollständig; dort etwas wegzulassen wäre das Verstecken von Daten.

*Warum das wichtig ist:* Vorher wurde das Konto **immer sofort** geschlossen. Wer am 5. den 30. als letzten Arbeitstag eintrug, sperrte den Mitarbeiter 25 Tage zu früh aus — während sein **Soll weiterlief**. Bei 18 Arbeitstagen sind das rund 144 Stunden Soll ohne Ist, die still vom Überstundenstand abgehen. **Beim Ausstellen werden zwei Dinge entfernt:** die Push-Abos der Geräte und die **Zwei-Faktor-Anmeldung** samt gemerkter Geräte. Letzteres, weil der Authenticator auf dem privaten Handy das Ausstellen sonst überlebt – käme der Account je versehentlich zurück, wäre das alte Handy sofort wieder ein gültiger zweiter Faktor, teils wochenlang ohne Code-Abfrage. Bei einer Wiedereinstellung wird er neu eingerichtet, wie beim Handywechsel. **Alles andere bleibt stehen** – Profilbild, Einzelrechte, Soll-Stunden, Start-Überstunden, Urlaubsanspruch, Personalnummer, Arbeitsbeginn, Geburtsdatum –, damit nach der Wiedereinstellung wirklich alles ist wie zuvor. *(Chef/Admin)*
...
- **Artikel einlernen** – darf einen unbekannten Barcode ins Verzeichnis aufnehmen (neues Produkt
  anlegen, einen Code an ein bestehendes anlernen, dabei eine Kategorie anlegen). Ohne dieses Recht
  endet ein unbekannter Scan mit einer **Auskunft** statt einer Eingabemaske: „Unbekannter Barcode
  … gib den Code deinem Admin durch." **Bestellen bleibt unverändert möglich** – als freier Text,
  wie vor dem Verzeichnis.
