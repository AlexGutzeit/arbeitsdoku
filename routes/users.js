...
router.get('/', authenticate, authorize('chef', 'buchhalter'), (req, res) => {
  ...
  if (req.user.role === 'admin') {
    users = db.prepare('SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date, active, created_at FROM users ORDER BY name').all();
  } else {
    users = db.prepare("SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date, active, created_at FROM users WHERE role != 'admin' ORDER BY name").all();
  }
...
router.get('/:id', authenticate, authorize('chef'), (req, res) => {
  ...
  user = db.prepare('SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date, created_at FROM users WHERE id = ?').get(req.params.id);
...
router.post('/', authenticate, authorize('chef'), async (req, res) => {
  const { username, password, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, hours_mon, hours_tue, hours_wed, hours_thu, hours_fri, personnel_no, work_start, birth_date } = req.body;
  ...
  const result = db.prepare(
    "INSERT INTO users (username, password_hash, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(username, hash, name, role, hpw, Number(start_overtime) || 0, planSelf, planAll, bulletin, upload, order, produkte, normPersonalNr(personnel_no) ?? null, beginnNeu || null, geburtNeu || null);
  ...
  const user = db.prepare('SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date, created_at FROM users WHERE id = ?').get(userId);
...
router.put('/:id', authenticate, authorize('chef'), (req, res) => {
  ...
  const { username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start } = req.body;
  ...
  // Produktrecht: eigene Rollenliste (OHNE Buchhalter) — deshalb eine eigene Zeile und nicht in
  // die #9-Zeile darueber hineingezogen.
  if (ROLLEN_MIT_PRODUKTRECHT.includes(effRole)) newProdukte = 0;
  ...
  db.prepare(
    'UPDATE users SET username=?, name=?, role=?, target_hours_per_week=?, start_overtime=?, can_plan=?, can_plan_all=?, can_bulletin=?, can_upload=?, can_order=?, can_products=?, personnel_no=?, work_start=?, birth_date=? WHERE id=?'
  ).run(
  ...
  const updated = db.prepare('SELECT id, username, name, role, target_hours_per_week, start_overtime, can_plan, can_plan_all, can_bulletin, can_upload, can_order, can_products, personnel_no, work_start, birth_date, created_at FROM users WHERE id = ?').get(req.params.id);
